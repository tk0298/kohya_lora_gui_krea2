...
それでもダメなら、gradient_checkpointing使用する。SDXL/Animaは必須。

Animaであれば、Block Swapを使用する。
...
GPU-ZやMSI AfterburnerのグラフでVRAM使用量が上限に近い状態で、GPU使用率100%張り付きで、消費電力が定格より大幅に低くなっているならVRAM不足。
...
※経験に基づく主観的な内容が含まれます。参考程度にとどめてください。また、この情報は古くなっている可能性があります。
...
SDXL以前は、dimの半分以下。dimの値に近いほど不安定になる。低すぎると学習能力が若干低下する。

Animaは、Dim(Rank)=AlphaでもOK。ただし予期せぬ画風変化を起こすことがあるので、dimの半分以下にしてもよい。

過学習を抑制する効果もある気がする。

なお、Alpha > Dimにしてもメリットはない。あれはLLMの話。
...
RAdamScheduleFree、ProdigyPlusScheduleFree、CAMEをおすすめする。
...
DiT世代のAnimaでさらに改善。CNNベースのSDXLごときが直線を書くのは難しい。
...
古いxformersで表示されることがあるが、なんの問題ない。そもそもxformersはsdpで代替可能で優位性も入れる必要もない。
...
Animaは画像の情報量に比例する傾向がある。情報量が少ないほど学習が速い。

## 最大トークン数は変えないほうがいい(SD1/SDXL)

この設定を150以上にすると、生成時に、プロンプトが76トークン以上になると品質が低下する。

## AnimaのTE学習はいる？

TEはゴールへの進行方向のヒントを与えるものなのでいらないかも。

しかし、ゴールに目的のものがあるとは限らないので、拡散モデルのみのLoRAで修正する。

## データセットのタグの集約っている?
クソ雑魚SD1が主流だった頃の古いテクニック、「トリガーワードに学習させたい要素を集約する」、いわゆる「マルゼン式」はしなくていいと思う。

プロンプト入力の手間を減らすメリット以外に、SDXL以前ではトークン数上限が75のため、それを超えないようトークン数を削減するメリットがある。

Animaでは512に拡張されたためメリットはあまりないどころか、集約するとトリガーワードの挙動がおかしくなるか、再現度が低下する可能性がある。そのままの方が衣装を(部分的に)変えたり髪型を変えたりと柔軟性があって良いぞ。

集約なしでも、生成時にトリガーワードだけで学習した人物の身体的特徴は再現できる。衣装は指定必要。面倒くさがらず都度指定すればいいだけ。

## LoRAを使用すると品質が低下する(SD1,SDXL)

LR、ステップ数、Dim(Rank)、Alphaのいずれかを下げると改善するかも。

しかし、LoRAの"ウェイトを二つの行列A,Bに分解して学習する"仕様が災いして何をしても品質を改善できないこともある。

v-prediction+ztsnrモデルで特に不安定。
...
## LoRAのファイルサイズに影響する設定
ネットワーク次元数(DimまたはRank)、Stable DiffusionはConv2d拡張の使用(使用時はconv dimも)、LyCORIS使用時はLyCORISのアルゴリズムの種類、Unet/Tencの有無、保存時の精度(--save_precision)、モデルの種類

## 作ったLoRAを使うと出力が暗くなったり赤みがかった色合いになる(SD1/SDXL)
...
## 学習阻害を目的としたノイズ(Adversarial Noise, いわゆるGlaze)のついた画像は避けるべき？
目立つレベルのノイズでなければデータセットに多少入っていても問題なし。気になるなら弱めのi2iかツールでぼかしをかけると良い。	ちなみに、Galzeはクソ雑魚SD1にすら効果がほぼない。
