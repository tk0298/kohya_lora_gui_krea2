...
## データセットのタグの集約っている?
「トリガーワードに学習させたい要素を集約する」、いわゆる「マルゼン式」はしなくていいと思う。

プロンプト入力の手間を減らすメリット以外に、SDXL以前ではトークン数上限が75のため、それを超えないようトークン数を削減するメリットがある。

Animaでは512に拡張されたためメリットはあまりない。むしろそのままの方が衣装を(部分的に)変えたり髪型を変えたりと柔軟性があって良いぞ。

集約なしでも、生成時にトリガーワードだけで学習した人物の身体的特徴は再現できる。衣装は指定必要。
