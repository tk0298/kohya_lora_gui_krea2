...
namespace Kohya_lora_trainer
{
    public partial class FormDatasetTools : Form
    {
        ...
        private void btnRemoveCapNoImg_Click(object sender, EventArgs e)
        {
            string sourceDir = tbxSourceDir.Text;
            int removedCnt = 0;
            int errorCount = 0;
            ...
            if (MessageBox.Show("画像だけでtxtがないものを同階層のtrashフォルダに移動します。よろしいですか。", "確認", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                lblProcessing.Visible = true;
                Update();
                //キャプションが入った場所
                ...
                foreach (string file in files)
                {
                    try
                    {
                        ...
                        if (ext == ".png" || ext == ".jpg" || ext == ".jpeg")
                        {
                            ...
                            if (!File.Exists(sourceDir + "\\" + imageFile + ".txt") && !File.Exists(sourceDir + "\\trash\\" + imageFile + ext))
                            {
                                if (!Directory.Exists(sourceDir + "\\trash"))
                                {
                                    Directory.CreateDirectory(sourceDir + "\\trash");
        ...
        private void btnRemoveImgLessPixels_Click(object sender, EventArgs e)
        {
            string sourceDir = tbxSourceDir.Text;
            int removedCnt = 0;
            int errorCount = 0;
            ...
            if (MessageBox.Show("画素数が指定未満の画像とキャプションを同階層のtrashフォルダに移動します。よろしいですか。", "確認", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                foreach (string file in files)
                {
                    try
                    {
                        ...
                        if (ext == ".png" || ext == ".jpg" || ext == ".jpeg")
                        {
                            Size imageSize = MyUtils.GetImageSize(file);
                            int imagePixels = imageSize.Width * imageSize.Height;

                            if (imagePixels > 0 && imagePixels < nudPixels.Value)
                            {
                                string path = sourceDir + "\\";
                                string imageFile = Path.GetFileNameWithoutExtension(file);
                                if (!File.Exists(sourceDir + "\\trash\\" + imageFile + ext))
        ...
        private void btnRemoveLessTags_Click(object sender, EventArgs e)
        {
            string sourceDir = tbxSourceDir.Text;
            //string boorutag = tbxBooruTag.Text;
            ...
            if (MessageBox.Show("txtファイル内のタグの数が閾値以下の場合に画像とtxtを同階層のtrashフォルダに移動させます。\r\nよろしいですか。", "確認", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                lblProcessing.Visible = true;
                Update();
                //キャプションが入った場所
                ...
                foreach (string file in files)
                {
                    try
                    {
                        ...
                        if (ext == ".txt")
                        {
                            ...
                            List<string> tags = new List<string>(txt.Split(", "));
                            if (tags.Count <= nudTagsCount.Value)
                            {
                                if (!File.Exists(sourceDir + "\\trash\\" + captionFile + ".txt"))
                                {
                                    //FileSystem.DeleteFileは重いので使わない
